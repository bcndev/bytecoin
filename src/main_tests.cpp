...
static void fix_merge_mining_tag(BlockTemplate &block) {
	if (block.major_version >= 2) {
		...
		if (!bytecoin::appendMergeMiningTagToExtra(block.parent_block.base_transaction.extra, mmTag))
			throw std::runtime_error("bytecoin::appendMergeMiningTagToExtra failed");
	}
}
