...
std::map<std::pair<uint32_t, bool>, P2PClientBasic::LevinHandlerFunction> P2PClientBasic::after_handshake_handlers = {
		...
		{{COMMAND_REQUEST_NETWORK_STATE::ID, false}, levinMethod<COMMAND_REQUEST_NETWORK_STATE::request>(&P2PClientBasic::on_msg_network_state)},
		{{COMMAND_REQUEST_NETWORK_STATE::ID, true}, levinMethod<COMMAND_REQUEST_NETWORK_STATE::response>(&P2PClientBasic::on_msg_network_state)},
		{{COMMAND_REQUEST_STAT_INFO::ID, false}, levinMethod<COMMAND_REQUEST_STAT_INFO::request>(&P2PClientBasic::on_msg_stat_info)},
		{{COMMAND_REQUEST_STAT_INFO::ID, true}, levinMethod<COMMAND_REQUEST_STAT_INFO::response>(&P2PClientBasic::on_msg_stat_info)},
#endif
		{{NOTIFY_NEW_BLOCK::ID, false}, levinMethod<NOTIFY_NEW_BLOCK::request>(&P2PClientBasic::on_msg_notify_new_block)},
		{{NOTIFY_NEW_TRANSACTIONS::ID, false}, levinMethod<NOTIFY_NEW_TRANSACTIONS::request>(&P2PClientBasic::on_msg_notify_new_transactions)},
		{{NOTIFY_REQUEST_TX_POOL::ID, false}, levinMethod<NOTIFY_REQUEST_TX_POOL::request>(&P2PClientBasic::on_msg_notify_request_tx_pool)},
		{{NOTIFY_REQUEST_CHAIN::ID, false}, levinMethod<NOTIFY_REQUEST_CHAIN::request>(&P2PClientBasic::on_msg_notify_request_chain)},
		{{NOTIFY_RESPONSE_CHAIN_ENTRY::ID, false}, levinMethod<NOTIFY_RESPONSE_CHAIN_ENTRY::request>(&P2PClientBasic::on_msg_notify_request_chain)},
		{{NOTIFY_REQUEST_GET_OBJECTS::ID, false}, levinMethod<NOTIFY_REQUEST_GET_OBJECTS::request>(&P2PClientBasic::on_msg_notify_request_objects)},
		{{NOTIFY_RESPONSE_GET_OBJECTS::ID, false}, levinMethod<NOTIFY_RESPONSE_GET_OBJECTS::request>(&P2PClientBasic::on_msg_notify_request_objects)}
};
...
void P2PClientBasic::send_timed_sync(){
	...
	BinaryArray msg = LevinProtocol::sendMessage(COMMAND_TIMED_SYNC::ID, LevinProtocol::encode(req), true);
...
Timestamp P2PClientBasic::get_local_time() const {
...
void P2PClientBasic::on_connect() {
	...
	BinaryArray msg = LevinProtocol::sendMessage(COMMAND_HANDSHAKE::ID, LevinProtocol::encode(req), true);
...
size_t P2PClientBasic::on_request_header(const BinaryArray &header, std::string &ban_reason) const {
	...
	return LevinProtocol::readCommandHeader(header, cmd, ban_reason);
}
...
void P2PClientBasic::msg_handshake(COMMAND_HANDSHAKE::request &&req) {
	...
	BinaryArray raw_msg = LevinProtocol::sendReply(COMMAND_HANDSHAKE::ID, LevinProtocol::encode(msg), 0);
	...
	std::cout << "P2p COMMAND_HANDSHAKE request version=" << int(req.node_data.version) << " unique_number=" << req.node_data.peer_id << " current_height=" << req.payload_data.current_height << std::endl;
...
void P2PClientBasic::msg_handshake(COMMAND_HANDSHAKE::response &&req) {
	...
	last_received_unique_number = req.node_data.peer_id;
	last_received_sync_data = req.payload_data;
	std::cout << "P2p COMMAND_HANDSHAKE response version=" << int(req.node_data.version) << " unique_number=" << req.node_data.peer_id << " current_height=" << req.payload_data.current_height << " local_peerlist.size=" << req.local_peerlist.size() << std::endl;
	on_msg_handshake(std::move(req));
	timed_sync_timer.once(TIMED_SYNC_TIMEOUT);
}
void P2PClientBasic::msg_ping(COMMAND_PING::request &&req) {
	...
	BinaryArray raw_msg = LevinProtocol::sendReply(COMMAND_PING::ID, LevinProtocol::encode(msg), 0);
	send(std::move(raw_msg));
	send_shutdown();
...
//	std::cout << "P2p COMMAND_TIMED_SYNC request height=" << req.payload_data.current_height << std::endl;
	...
	BinaryArray raw_msg = LevinProtocol::sendReply(COMMAND_TIMED_SYNC::ID, LevinProtocol::encode(msg), 0);
	send(std::move(raw_msg));
...
void P2PClientBasic::on_request_ready() {
	...
	while (read_next_request(header, body)) {
		try {
			...
			if (LevinProtocol::readCommandHeader(header, cmd, ban_reason) == std::string::npos) {
				disconnect(ban_reason);
				return;
			}
			if (!handshake_ok()) {
				auto ha = before_handshake_handlers.find({cmd.command, cmd.isResponse});
			...
			auto ha = after_handshake_handlers.find({cmd.command, cmd.isResponse});
			...
			std::cout << "generic bytecoin::P2P cmd={" << cmd.command << "} " << cmd.isResponse << " " << cmd.isNotify << " {" << "}" << std::endl;
