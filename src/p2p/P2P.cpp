...
P2PClient::P2PClient(size_t header_size, bool incoming, D_handler d_handler) :
		sock([this](bool canread, bool canwrite) { advance_state(true); }, std::bind(&P2PClient::on_socket_disconnect, this)),
		incoming(incoming),
...
void P2PClient::read(bool called_from_runloop) {
	if (!receiving_body) {
		...
		if (request_body_length == std::string::npos) {
			//log(Logging::WARNING) << "P2PClient::read got bad message from " << address.get_address() << ":" << address.get_port() << std::endl;
...
void P2PClient::advance_state(bool called_from_runloop) {
	write();
	if (responses.size() > 1) // TODO - keep track of total number of bytes to send, read new data when that number is low enough
...
//        std::cout << "Server::accept=" << std::endl;
	...
	while (true) {
		if (!next_client[incoming]) {
			...
			next_client[incoming]->d_handler = std::bind(&P2P::on_client_disconnected, this, next_client[incoming].get(), _1);
		}
		std::string addr;
...
bool P2P::connect_one(const NetworkAddress &address) {
	...
	P2PClient *who = next_client[incoming].get();
	clients[incoming][next_client[incoming].get()] = std::move(next_client[incoming]);
	log(logging::INFO) << "Connecting to=" << common::ip_address_and_port_to_string(address.ip, address.port) << std::endl;
	who->on_connect();
	return true;
}
...
void P2P::connect_all_nodelay() {
	...
	while (clients[incoming].size() < config.p2p_default_connections_count) { // Some clients are not connected
		...
		if (!peers.get_peer_to_connect(best_address, connected, get_local_time())) {
			log(logging::INFO) << "No peers to connect to, will try again after " << RECONNECT_TIMEOUT << " seconds" << std::endl;
		...
		if (immediate_attempts >= config.p2p_default_connections_count) {  // When network is not reachable, we try just a handfull times
			log(logging::INFO) << "Connect repeatedly fails, will try again after " << RECONNECT_TIMEOUT << " seconds" << std::endl;
			reconnect_timer.once(RECONNECT_TIMEOUT);
			return;
		}
	}
...
P2P::P2P(logging::ILogger &log, const Config &config, PeerDB &peers, client_factory c_factory)
	...
	try {
		la_socket.reset(new platform::TCPAcceptor{config.p2p_bind_ip, config.p2p_bind_port, std::bind(&P2P::accept_all, this)});
	} catch (const std::runtime_error &r) {
		this->log(logging::WARNING) << " failed to create listening socket, what=" << r.what() << ", working with outbound connections only" << std::endl;
	}
	connect_all();
	accept_all();
}
...
uint32_t P2P::get_p2p_time() const {
	return get_local_time();
}

uint32_t P2P::get_local_time() const {
