...
Hash ProofOfTrustLegacy::get_hash() const {
	std::string s;
...
void ser_members(network_config &v, seria::ISeria &s) {
...
void ser_members(basic_node_data &v, seria::ISeria &s) {
...
void ser_members(CORE_SYNC_DATA &v, seria::ISeria &s) {
	if (s.is_input()) {
		uint32_t on_wire = 0;
...
void ser_members(COMMAND_HANDSHAKE::request &v, seria::ISeria &s) {
	seria_kv("node_data", v.node_data, s);
	seria_kv("payload_data", v.payload_data, s);
}

void ser_members(COMMAND_HANDSHAKE::response &v, seria::ISeria &s) {
	seria_kv("node_data", v.node_data, s);
	seria_kv("payload_data", v.payload_data, s);
	serialize_as_binary(v.local_peerlist, "local_peerlist", s);
}

void ser_members(COMMAND_TIMED_SYNC::request &v, seria::ISeria &s) { seria_kv("payload_data", v.payload_data, s); }

void ser_members(COMMAND_TIMED_SYNC::response &v, seria::ISeria &s) {
	seria_kv("local_time", v.local_time, s);
	seria_kv("payload_data", v.payload_data, s);
	serialize_as_binary(v.local_peerlist, "local_peerlist", s);
}

void ser_members(COMMAND_PING::request &v, seria::ISeria &s) {}

void ser_members(COMMAND_PING::response &v, seria::ISeria &s) {
	seria_kv("status", v.status, s);
	seria_kv("peer_id", v.peer_id, s);
}

#if bytecoin_ALLOW_DEBUG_COMMANDS
void ser_members(ProofOfTrustLegacy &v, seria::ISeria &s) {
	seria_kv("peer_id", v.peer_id, s);
	seria_kv("time", v.time, s);
	seria_kv("sign", v.sign, s);
}

void ser_members(COMMAND_REQUEST_STAT_INFO::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }

void ser_members(COMMAND_REQUEST_STAT_INFO::response &v, seria::ISeria &s) {
	seria_kv("version", v.version, s);
...
void ser_members(RawBlockLegacy &v, seria::ISeria &s) {
	...
	if (s.is_input()) {
		...
		std::transform(other_transactions.begin(), other_transactions.end(), std::back_inserter(v.transactions),
		    [](const std::string &s) { return BinaryArray(s.data(), s.data() + s.size()); });
	} else {
...
void ser_members(NOTIFY_NEW_BLOCK::request &v, seria::ISeria &s) {
	seria_kv("b", v.b, s);
	seria_kv("current_blockchain_height", v.current_blockchain_height, s);
	seria_kv("hop", v.hop, s);
}

void ser_members(NOTIFY_NEW_TRANSACTIONS::request &v, seria::ISeria &s) {
...
void ser_members(NOTIFY_REQUEST_GET_OBJECTS::request &v, seria::ISeria &s) {
	serialize_as_binary(v.txs, "txs", s);
	serialize_as_binary(v.blocks, "blocks", s);
}

void ser_members(NOTIFY_RESPONSE_GET_OBJECTS::request &v, seria::ISeria &s) {
...
void ser_members(NOTIFY_REQUEST_CHAIN::request &v, seria::ISeria &s) {
	serialize_as_binary(v.block_ids, "block_ids", s);
}

void ser_members(NOTIFY_RESPONSE_CHAIN_ENTRY::request &v, seria::ISeria &s) {
...
void ser_members(NOTIFY_REQUEST_TX_POOL::request &v, seria::ISeria &s) { serialize_as_binary(v.txs, "txs", s); }
}
