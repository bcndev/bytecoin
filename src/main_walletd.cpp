...
Options:
  ...
  --create-legacy-wallet                Create wallet file with new random keys, then exit. Must be used with --wallet-file option.
  --import-keys                         Create wallet file with imported keys read as a line from stdin. Must be used with --create-legacy-wallet.
  --create-wallet                       Create wallet file with existing BIP39 mnemonic read as a line from stdin.
  --creation-timestamp=<t>              When creating wallet file, set wallet creation timestamp to <t> (now is possible value) [default: 0]. Must be used with --import-keys or --create-wallet option.
  --address-type=auditable              Create an auditable wallet instead of standard wallet. Must be used with --create-wallet.
  --address-count=<c>                   When creating wallet file, immediately generate <c> addresses [default: 1].
  --create-mnemonic                     Create a new random BIP39 mnemonic. If --create-wallet is specified, create wallet from mnemonic, otherwise print mnemonic and exit.
  --mnemonic-strength=<bits>            Used with --create-mnemonic, [default: 256].
  --secrets-via-api                     Specify to allow getting secrets using 'get_wallet_info' json RPC method.
  --set-password                        Read new password as a line from stdin (twice) and re-encrypt wallet file, then exit.
  --launch-after-command                Instead of exiting, continue launching after --create-wallet, --create-legacy-wallet and --set-password commands
...
int main(int argc, const char *argv[]) try {
	...
	std::string wallet_file, password, new_password, walletd_http_auth, export_view_only, import_keys_value,
	    backup_wallet;
	...
	if (const char *pa = cmd.get("--address-type")) {
		if (std::string(pa) == "auditable" || std::string(pa) == "2")
			address_type = 2;
		else if (std::string(pa) == "standard" || std::string(pa) == "1")
			address_type = 1;
		else {
			std::cout << "If specified, address type must be 'auditable' or 'standard'" << std::endl;
			return 1;
		}
	}
	...
	if (cmd.get_bool("--create-mnemonic")) {
		...
		mnemonic = cn::Bip32Key::create_random_bip39_mnemonic(bits);
		if (!create_wallet) {
			std::cout << mnemonic << std::endl;
			return 0;
		}
		creation_timestamp = platform::now_unix_timestamp();  // We know, we know...
	}
	if (create_wallet || create_legacy_wallet) {
	...
	if (import_keys && !create_legacy_wallet) {
		std::cout
		    << "When importing keys, you should use --create-legacy-wallet. You cannot import into existing wallet."
		    << std::endl;
		return api::WALLETD_WRONG_ARGS;
	}
	...
	if (const char *pa = cmd.get("--export-view-only")) {
		if (import_keys || create_wallet || create_legacy_wallet || export_keys) {
			std::cout
		...
		export_view_only        = pa;
		view_outgoing_addresses = cmd.get_bool("--view-outgoing-addresses");
	}
	if (const char *pa = cmd.get("--backup-wallet", "Deprecated, use --backup-wallet-data"))
		backup_wallet = pa;
	if (const char *pa = cmd.get("--backup-wallet-data"))
		backup_wallet = platform::normalize_folder(pa);
	if (const char *pa = cmd.get("--wallet-password")) {  // Undocumented, usefull for debugging
		password     = pa;
		ask_password = false;
	}
	if (const char *pa = cmd.get("--walletd-http-auth")) {  // Undocumented, usefull for debugging
		walletd_http_auth     = pa;
		ask_walletd_http_auth = false;
	}
	if (!ask_password && (create_wallet || create_legacy_wallet)) {
	...
	if (create_legacy_wallet && import_keys) {
	...
	if (!config.bytecoind_remote_port && !create_wallet && !create_legacy_wallet && !set_password && !export_keys &&
	    export_view_only.empty() && backup_wallet.empty()) {
	...
	if (!create_wallet && !create_legacy_wallet && ask_password) {
	...
	if (create_wallet || create_legacy_wallet || set_password) {
	...
	try {
		if (create_wallet) {
			wallet = std::make_unique<WalletHD>(currency, logManagerWalletNode, wallet_file, new_password, mnemonic,
			    address_type, creation_timestamp, mnemonic_password);
			wallet->create_look_ahead_records(address_count);
		} else if (create_legacy_wallet) {
			...
			const bool readonly  = !backup_wallet.empty() || !export_view_only.empty() || export_keys;
			const bool is_sqlite = WalletHD::is_sqlite(wallet_file);
			if (is_sqlite)
				wallet = std::make_unique<WalletHD>(currency, logManagerWalletNode, wallet_file, password, readonly);
			else
				wallet =
				    std::make_unique<WalletContainerStorage>(currency, logManagerWalletNode, wallet_file, password);
		}
	...
	try {
		if (!backup_wallet.empty()) {
			...
			wallet->backup(dst_name, set_password ? new_password : password);
			common::console::set_text_color(common::console::BrightRed);
			std::cout
		...
		if (!export_view_only.empty()) {
			...
			wallet->export_wallet(
			    export_view_only, set_password ? new_password : password, true, view_outgoing_addresses);
			std::cout << "Successfully exported view-only copy of the wallet" << std::endl;
			return 0;
		}
		if (export_keys) {
			if (!wallet->is_deterministic() && wallet->get_records().size() != 1)
	...
	if (ask_walletd_http_auth) {
		std::cout << "Enter HTTP authorization <user>:<password> for walletd RPC: " << std::flush;
		if (!console_setup.getline(walletd_http_auth, true)) {
			std::cout << "Unexpected end of stdin" << std::endl;
			return api::WALLETD_WRONG_ARGS;
		}
		std::cout << std::endl;
	}
	boost::algorithm::trim(walletd_http_auth);
	config.walletd_authorization = common::base64::encode(
	    BinaryArray(walletd_http_auth.data(), walletd_http_auth.data() + walletd_http_auth.size()));
	if (config.walletd_authorization.empty()) {
		...
		if (walletd_http_auth.find(":") == std::string::npos) {
