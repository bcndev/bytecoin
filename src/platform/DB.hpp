...
public:
	explicit DB(const std::string &full_path, uint64_t max_db_size = 0x8000000000); // 0.5 Tb default, out of total 4 Tb on windows
	...
	class Cursor {
		...
		friend class DB;
		Cursor(lmdb::Cur &&db_cur, const std::string &prefix, const std::string &middle, size_t max_key_size, bool forward);
	public:
