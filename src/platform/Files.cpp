...
FileStream::FileStream(const std::string &filename, OpenMode mode) {
	...
	handle = CreateFileW(wfilename.c_str(), GENERIC_READ | (mode == READ_EXISTING ? 0 : GENERIC_WRITE), FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, nullptr, (mode == TRUNCATE_READ_WRITE) ? CREATE_ALWAYS : (mode == READ_WRITE_EXISTING) ? OPEN_EXISTING : OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
uint64_t FileStream::seek(uint64_t pos, int whence){
	...
	lpos.QuadPart = pos;
	static_assert(SEEK_END == FILE_END && SEEK_CUR == FILE_CURRENT && SEEK_SET == FILE_BEGIN, "Whene definition between Windows and POSIX do not match");
...
std::wstring FileStream::utf8_to_utf16(const std::string & str) {
	...
	auto si = MultiByteToWideChar(CP_UTF8, 0, str.data(), boost::lexical_cast<int>(str.size()), &result[0], boost::lexical_cast<int>(result.size()));
	result.resize(si);
	return result;
}
...
std::string FileStream::utf16_to_utf8(const std::wstring & str) {
	...
	auto si = WideCharToMultiByte(CP_UTF8, 0, str.data(), boost::lexical_cast<int>(str.size()), &result[0], boost::lexical_cast<int>(result.size()), nullptr, nullptr);
	result.resize(si);
	return result;
}
