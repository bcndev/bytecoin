...
WalletPreparatorMulticore::WalletPreparatorMulticore(){
	auto th_count = std::max<size_t>(2, 3 * std::thread::hardware_concurrency() / 4); // we use more energy but have the same speed when using hyperthreading to max
	std::cout << "Starting multicore transaction preparator using " << th_count << "/" << std::thread::hardware_concurrency() << " cpus" << std::endl;
...
PreparedWalletTransaction::PreparedWalletTransaction(Transaction && ttx, const SecretKey & view_secret_key):tx(std::move(ttx)){
	...
	for (const auto &output : tx.outputs) {
		if (output.target.type() == typeid(KeyOutput)) {
			...
			PublicKey spend_key;
			underive_public_key(derivation, key_index, key_output.key, spend_key); // error indicated by spend_key == PublicKey{}
...
PreparedWalletBlock::PreparedWalletBlock(Block && block, const SecretKey & view_secret_key){
	header = block.header;
	base_transaction = PreparedWalletTransaction(std::move(block.header.base_transaction), view_secret_key);
	transactions.reserve(block.transactions.size());
	for (size_t tx_index = 0; tx_index != block.transactions.size(); ++tx_index) {
		transactions.emplace_back(std::move(block.transactions.at(tx_index)), view_secret_key);
	}
}
...
void WalletPreparatorMulticore::thread_run(){
	while(true){
		...
		PreparedWalletBlock result(std::move(sync_block.block), view_secret_key);
		{
			std::unique_lock<std::mutex> lock(mu);
...
void WalletPreparatorMulticore::start_work(const api::bytecoind::SyncBlocks::Response & new_work, const SecretKey & view_secret_key){
	std::unique_lock<std::mutex> lock(mu);
	work = new_work;
...
std::string toBinaryKey(const T &s) {
	...
	return DB::to_binary_key((const unsigned char *)&s, sizeof(s));
}

void WalletState::DeltaState::redo_transaction(Height, const Hash &tid, const Transaction &tx,
                                               const api::Transaction &ptx) {
...
void WalletState::DeltaState::redo_keyimage_output(const api::Output &output, Height block_height, Timestamp block_unlock_timestamp) {
	m_unspents[output.public_key].push_back(output);
}

void WalletState::DeltaState::undo_keyimage_output(const api::Output &output) {
	throw std::logic_error("DeltaState::undo_keyimage_output");  // We do not call it on memory states
}
...
void WalletState::DeltaState::undo_height_keyimage(Height height, const KeyImage &keyimage) {
	...
	kit->second -= 1;
	if( kit->second < 0)
		std::cout << "DeltaState::undo_height_keyimage more keyimages undone than redone 2" << std::endl;
	if( kit->second <= 0)
...
void WalletState::DeltaState::undo_transaction(const Hash &tid) {
	...
	if (tit == m_transactions.end())
		return;
	const Transaction &tx = tit->second.first;
	for (const auto &output : tx.outputs) {
		if (output.target.type() == typeid(KeyOutput)) {
			const KeyOutput &key_output = boost::get<KeyOutput>(output.target);
			...
			uit->second.pop_back(); // We can pop wrong output, but this is not important - situation arises only in pool and only during attack
			if( uit->second.empty() )
...
WalletState::WalletState(Wallet &wallet, logging::ILogger &log, const Config &config, const Currency &currency)
    ...
    , m_log(log)
    , m_wallet(wallet)
    , m_db(config.get_coin_directory("wallet_cache") + "/" + wallet.get_cache_name(), 0x2000000000) // 128 gb
    , m_memory_state(0, 0) {
    std::string version;
    m_db.get("$version", version);
    if (version != version_current) {
        std::cout << "Data format changed, old version=" << version << " current version=" << version_current << ", deleting wallet cache..." << std::endl;
        for( DB::Cursor cur = m_db.rbegin(std::string()); !cur.end(); cur.erase() ){
        }
        m_db.put("$version", version_current, true);
    }
	if (!read_tips()) {
		BinaryArray ba = seria::toBinary(m_genesis_bid);
...
void WalletState::wallet_addresses_updated() {
	...
	// We never delete from ADDRESSES_PREFIX index, because it correctly reflects scanned outputs, their spendable and
	// balances
...
bool WalletState::sync_with_blockchain(api::bytecoind::SyncBlocks::Response &resp) {
	if (resp.blocks.empty())  // Our creation timestamp > last block timestamp, so no blocks
		return true;
	...
	while (m_tip_height + 1 < resp.start_height + resp.blocks.size()) {
		...
		if (m_tip_height + 1 != m_tail_height && header.previous_block_hash != m_tip.hash)
			return false;
		if (header.timestamp >= m_wallet.get_oldest_timestamp()) {
			const auto &block_gi = resp.blocks.at(bin).global_indices;
//			bool our_block = (m_tip_height + 1 == 1319239) || (m_tip_height + 1 == 1319242) || (m_tip_height + 1 == 1321529);
//			if( our_block )
//				test_print_everything("Before redo_block");
//			redo_block(header, block, block_gi, m_tip_height + 1);
//			push_chain(header);
//			if( our_block )
//				test_print_everything("Before undo_block");
//			undo_block(m_tip_height);
//			pop_chain();
//			if( our_block )
//				test_print_everything("After undo_block");
...
bool WalletState::sync_with_blockchain(const api::bytecoind::SyncMemPool::Response &resp) {
	...
	for (size_t i = 0; i != resp.added_binary_transactions.size(); ++i) {
		Transaction tx;
		seria::fromBinary(tx, resp.added_binary_transactions[i]);
		...
		if (!redo_transaction(pwtx, global_indices, &m_memory_state, false, tid, Hash{}, resp.added_transactions.at(i).timestamp)){
		}
	}
...
void WalletState::add_transient_transaction(const Hash &tid, const Transaction &tx) {
	if (!m_pool_hashes.insert(tid).second) {  // Already there
		return;
	}
	std::vector<uint32_t> global_indices(tx.outputs.size(), 0);
	PreparedWalletTransaction pwtx(Transaction(tx), m_wallet.get_view_secret_key());
...
bool WalletState::redo_block(const api::BlockHeader &header, const PreparedWalletBlock &pb,
    ...
    // order is important here. Unlock before redo, or will lock/unlock twice -> invariant dead
	...
	Hash base_hash = get_transaction_hash(pb.base_transaction.tx);
	if (!redo_transaction(pb.base_transaction, global_indices[0], &delta_state, true, base_hash, header.hash,
	                      pb.header.timestamp)) {
    } // Just ignore
	bool our_block = (height == 1319239 || height == 1319242 || height == 1321529 || height == 1340382 ||
	                  height == 1341774 || height == 1355639 || height == 1357349);
	if (our_block) {
		//		AccountPublicAddress address{wallet.getRecords().begin()->first, wallet.getViewPublicKey()};
		//		auto bal = get_balance(address, height);
		//		std::cout << "Our block! height=" << height << " balance= " << currency.formatAmount(bal.total()) <<
		// std::endl;
	}
	for (size_t tx_index = 0; tx_index != pb.transactions.size(); ++tx_index) {
		const Hash tid = pb.header.transaction_hashes.at(tx_index);
		if( m_pool_hashes.erase(tid) != 0 ) {
//			std::cout << "RACE remove tx in redo_block tx=" << common::pod_to_hex(tid) << std::endl;
		}
		m_memory_state.undo_transaction(tid);
		if (!redo_transaction(pb.transactions.at(tx_index), global_indices.at(tx_index + 1), &delta_state, false,
							  tid, header.hash, pb.header.timestamp)){
		} // just ignore
	}
	...
	if (our_block) {
		//		AccountPublicAddress address{wallet.getRecords().begin()->first, wallet.getViewPublicKey()};
		//		auto bal = get_balance(address, height);
		//		std::cout << "Height = " << height << " balance= " << currency.formatAmount(bal.total()) << std::endl;
	}
	return true;
}
...
void WalletState::undo_block(Height height) {
	...
	for (DB::Cursor cur = m_db.begin(prefix); !cur.end(); cur.erase()) {
		api::Output output;
		seria::fromBinary(output, cur.get_value_array());
    ...
    if( !read_chain(height - 1, prev_header))
        return; // If we are just undone tail block, there should be no outputs, so nothing to lock
    lock_unlock(height - 1, height, prev_header.timestamp_unlock, m_tip.timestamp_unlock, true);
}

bool WalletState::parse_raw_transaction(api::Transaction & ptx, const Transaction & tx, Hash tid)const{
	std::vector<uint32_t> global_indices(tx.outputs.size(), 0);
	Amount output_amount;
	PreparedWalletTransaction pwtx(Transaction(tx), m_wallet.get_view_secret_key() );
	if( !parse_raw_transaction(ptx, output_amount, pwtx, tid, global_indices, get_tip_height(), get_tip().timestamp_unlock)) // TODO +1 ?
		return false;
	Amount input_amount  = 0;
	api::Transfer input_transfer; // We do not know "from" addresses, so leave address empty
...
bool WalletState::parse_raw_transaction(api::Transaction & ptx, Amount & output_amount, const PreparedWalletTransaction & pwtx, Hash tid, const std::vector<uint32_t> &global_indices, Height block_height, Timestamp block_unlock_timestamp)const{
	if (global_indices.size() != pwtx.tx.outputs.size())  // Bad node
		return false;
	const Transaction & tx = pwtx.tx;
	PublicKey tx_public_key = getTransactionPublicKeyFromExtra(tx.extra);
	...
	const bool tx_unlocked = m_currency.is_transaction_spend_time_unlocked(ptx.unlock_time, block_height, block_unlock_timestamp);
			...
			if( spend_key != PublicKey{} ){
				...
				if (sk != m_wallet.get_records().end()) {
					...
					if (derive_public_key(pwtx.derivation, out_index, spend_key, in_ephemeral.publicKey)) {
						derive_secret_key(pwtx.derivation, out_index, sk->second.spend_secret_key, in_ephemeral.secretKey);
//						std::cout << "My output! outIndex=" << out_index << " amount=" << output.amount << std::endl;
...
bool WalletState::redo_transaction(const PreparedWalletTransaction &pwtx, const std::vector<uint32_t> &global_indices,
	...
	if( !parse_raw_transaction(ptx, output_amount, pwtx, tid, global_indices, delta_state->get_block_height(), delta_state->get_unlock_timestamp()))
		return false;
...
void WalletState::undo_transaction(const Transaction &) {}
...
bool WalletState::read_tips() {
	...
	seria::fromBinary(other_genesis_bid, rb);
	if (m_genesis_bid != other_genesis_bid)
		throw std::runtime_error("Database holds different genesis bid");  // TODO - return error or clear DB
	std::string val1;
...
void WalletState::push_chain(const api::BlockHeader &header) {
	m_tip_height += 1;
	BinaryArray ba = seria::toBinary(header);
...
void WalletState::redo_transaction(Height height, const Hash &tid, const Transaction &tx, const api::Transaction &ptx) {
	...
	BinaryArray str_pa = seria::toBinary(std::make_pair(tx, ptx));
	BinaryArray str = seria::toBinary(ptx);
...
//	std::cout << "Lock am=" << output.amount / 1E8 << " gi=" << output.global_index << " un=" << output.unlock_time << std::endl;
	BinaryArray ba = seria::toBinary(output);
...
//	std::cout << "Unlock am=" << output.amount / 1E8 << " gi=" << output.global_index << " un=" << output.unlock_time << std::endl;
	std::string unkey;
	uint32_t clamped_unlock_time = static_cast<uint32_t>(std::min<UnlockMoment>(output.unlock_time, 0xFFFFFFFF));
	if (m_currency.is_transaction_spend_time_block(output.unlock_time))
		unkey = UNLOCK_BLOCK_PREFIX + DB::to_ascending_key(clamped_unlock_time) + "/" + std::to_string(output.amount) +
...
//		std::cout << "Add unspent am=" << output.amount / 1E8 << " gi=" << output.global_index
//		          << " un=" << output.unlock_time << std::endl;
	...
	BinaryArray ba2 = seria::toBinary(output);
...
//	if (output.unlock_time)
//		std::cout << "Remove unspent am=" << output.amount / 1E8 << " gi=" << output.global_index
//		          << " un=" << output.unlock_time << std::endl;
	modify_balance(output, 0, -1);
	auto keyuns = UNSPENT_HEIGHT_PREFIX + output.address + "/" + DB::to_ascending_key(output.height) + "/" +
	              std::to_string(output.amount) + "/" + std::to_string(output.global_index);
...
void WalletState::modify_balance(const api::Output &output, int locked_op, int spendable_op) {
	...
	if (m_db.get(bakey, ba))
		seria::fromBinary(balance, ba);
	if (m_db.get(bakey2, ba))
		seria::fromBinary(balance2, ba);
//	std::cout << "modify_balance " << output.amount << " lockedOp=" << locked_op << " spendableOp=" << spendable_op << std::endl;
	...
	if (balance.total() == 0)
		...
		m_db.put(bakey, seria::toBinary(balance), false);
	if (balance2.total() == 0)
		m_db.del(bakey2, false);
	else
		m_db.put(bakey2, seria::toBinary(balance2), false);
}

// Add new unspent
void WalletState::redo_keyimage_output(const api::Output &output, Height block_height, Timestamp block_unlock_timestamp) {
	BinaryArray ba = seria::toBinary(output);

	auto kikey = KEYIMAGE_PREFIX + toBinaryKey(output.key_image);
	BinaryArray ba2;
	if (m_db.get(kikey, ba2)) {
		// Protect against an attack with 2 identical output.public_key (and hence keyimages) in 2 different transactions
		return;
	}
...
void WalletState::undo_keyimage_output(const api::Output &output) {
	...
	seria::fromBinary(was_output, ba2);
	if (output.amount != was_output.amount || output.global_index != was_output.global_index){
		// Protect against an attack with 2 identical output.public_key (and hence keyimages) in 2 different transactions
		// Subsequent outputs cannot have the same global_index, as global_index is incremented on each output
		return;
	}
	m_db.del(kikey, true);
...
// Unique in that it reads (begin, end] interval, not [begin, end) as most other funs. That is because block height 312
...
void WalletState::lock_unlock(Height prev_height, Height now_height, Timestamp prev, Timestamp now, bool lock) {
	...
	read_unlock_index(outputs, UNLOCK_BLOCK_PREFIX, prev_height + m_currency.locked_tx_allowed_delta_blocks, now_height + m_currency.locked_tx_allowed_delta_blocks);
	read_unlock_index(outputs, UNLOCK_TIME_PREFIX, prev + m_currency.locked_tx_allowed_delta_seconds, now + m_currency.locked_tx_allowed_delta_seconds);
...
void WalletState::redo_height_keyimage(Height height, const KeyImage &keyimage) {
	...
	if (m_db.get(key, rb)) {
		...
		// Code below is used temporarily to allow spending locked outputs (due to changes to unlock code in new
		// version)
		auto keyuns = UNSPENT_HEIGHT_PREFIX + output.address + "/" + DB::to_ascending_key(output.height) + "/" +
		              std::to_string(output.amount) + "/" + std::to_string(output.global_index);
		...
		m_db.put(hekey, seria::toBinary(was_unlocked), true);
	}
}
...
void WalletState::undo_height_keyimage(Height height, const KeyImage &keyimage) {
	...
	seria::fromBinary(was_unlocked, ba);

	if (was_unlocked)
		add_to_unspent_index(output);
	//	auto hekey = HEIGHT_KEYIMAGE_PREFIX + DB::to_ascending_key(height) + "/" + DB::to_binary_key(keyimage.data,
	// sizeof(keyimage.data));
	//	db.del(hekey, true); // Removed during iteration in undo_block
}

std::vector<api::Output> WalletState::api_get_unspent(const std::string &address, Height height) const {
	...
	for (DB::Cursor cur = m_db.begin(prefix); !cur.end(); cur.next()) {
		...
		if (DB::from_ascending_key(shei) > height)
			break;
		api::Output item;
		seria::fromBinary(item, cur.get_value_array());
...
std::vector<api::Output> WalletState::api_get_locked_or_unconfirmed_unspent(const std::string &address,
	...
	for (DB::Cursor cur = m_db.begin(prefix, middle); !cur.end(); cur.next()) {
		std::string shei, rest;
		if (!common::split_string(cur.get_suffix(), "/", shei, rest))
			throw std::logic_error("Invariant dead api_get_locked_or_unconfirmed_unspent index corrupted");
		api::Output item;
		seria::fromBinary(item, cur.get_value_array());
		if (!m_memory_state.is_spent(item))
			result.push_back(item);
...
std::vector<api::Block> WalletState::api_get_transfers(const std::string &address, Height & from_height,
													   Height & to_height, bool forward, uint32_t desired_tx_count) const {
	...
	for (DB::Cursor cur = forward ? m_db.begin(prefix, middle) : m_db.rbegin(prefix, middle); !cur.end(); cur.next()) {
		...
		if (!forward && height <= from_height)
			break;
		api::Transaction tx;
		seria::fromBinary(tx, cur.get_value_array());
...
bool WalletState::api_get_transaction(Hash tid, Transaction & tx, api::Transaction & ptx) const{
	auto trkey   = TRANSACTION_PREFIX + toBinaryKey(tid);
	BinaryArray data;
	if (!m_db.get(trkey, data))
		return false;
...
bool WalletState::api_create_proof(SendProof & sp)const{
	...
	if( !crypto::generate_send_proof(tx_keys.publicKey, tx_keys.secretKey, sp.address.view_public_key, sp.derivation, message_hash, sp.signature))
		return false;
...
std::vector<api::Transaction> WalletState::api_list_history(const std::string &address, Hash start_transaction,
	...
	for (; !cur.end(); cur.next()) {
		...
		seria::fromBinary(pa2, cur.get_value_array());
...
api::Balance WalletState::get_balance(const std::string &address, Height height) const {
	...
	auto unlocked_outputs = api_get_unlocked_outputs(address, height, m_tip_height);
	for (auto &&ou : unlocked_outputs) { // TODO - correct behaviour of memory-pool-spent unlocked outputs
