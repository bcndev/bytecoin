...
public:
	...
	virtual uint32_t push_amount_output(Amount, UnlockMoment, Height block_height, Timestamp block_unlock_timestamp, const PublicKey &) = 0;
...
public:
	...
	~RingCheckerMulticore();
    void cancel_work();
    std::string start_work_get_error(IBlockChainState * state, const Currency & currency, const Block & block, Height unlock_height, Timestamp unlock_timestamp); // can fail immediately
...
public:
	...
	bool create_mining_block_template(BlockTemplate &, const AccountPublicAddress &, const BinaryArray &extra_nonce,
	                                  Difficulty &, Height &) const;
...
protected:
	std::string get_standalone_consensus_error(const PreparedBlock & pb, api::BlockHeader &info,
	                                           const api::BlockHeader &prev_info) const;
	virtual bool check_standalone_consensus(const PreparedBlock & pb, api::BlockHeader &info,
	                                        const api::BlockHeader &prev_info) const override;
...
private:
	...
	public:
		...
		virtual uint32_t push_amount_output(Amount, UnlockMoment, Height block_height, Timestamp block_unlock_timestamp, const PublicKey &) override;
	...
	mutable std::unordered_map<Amount, uint32_t> m_next_gi_for_amount;  // Read from db on first use, write on modification

	void update_first_seen_timestamp(const Hash & tid, Timestamp now); // 0 to delete

	BroadcastAction add_transaction(const Hash & tid, const Transaction & tx, Height unlock_height, Timestamp unlock_timestamp, size_t max_pool_complexity, bool check_sigs);
	void add_to_pool(Hash tid, const Transaction & tx);
	...
	void calculate_consensus_values(Height height_delta, uint32_t & next_median_size, Timestamp & next_median_timestamp, Timestamp & next_unlock_timestamp)const;
