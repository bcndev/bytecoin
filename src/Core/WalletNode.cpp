...
WalletNode::HandlersMap WalletNode::m_jsonrpc3_handlers = {
    ...
    {api::walletd::SendTransaction::method(),
     json_rpc::makeMemberMethodSeria(&WalletNode::handle_send_transaction3)},
...
bool WalletNode::process_json_rpc_request(const HandlersMap &handlers, http::Client *who, http::RequestData &&request,
                                          http::ResponseData &response, bool &method_found) {
	method_found = false;
	...
	try {
		...
		json_resp.setId(json_req.getId());  // copy id

		auto it = handlers.find(json_req.getMethod());
		if (it == handlers.end()) {
			return false;
//			m_log(logging::INFO) << "json request method not found - " << json_req.getMethod() << std::endl;
//			throw json_rpc::Error(json_rpc::errMethodNotFound);
		}
		method_found = true;
//		m_log(logging::INFO) << "json request method=" << json_req.getMethod() << std::endl;
		bool result = it->second(this, who, std::move(request), std::move(json_req), json_resp);
		if (!result)
			return false;

	} catch (const json_rpc::Error &err) {
		json_resp.setError(err);
	} catch (const std::exception &e) {
		json_resp.setError(json_rpc::Error(json_rpc::errInternalError, e.what()));
	}

	response.setBody(json_resp.getBody());
...
bool WalletNode::handle_get_status3(http::Client *who, http::RequestData &&raw_request,
                                    ...
                                    api::walletd::GetStatus::Request &&request,
                                    api::walletd::GetStatus::Response &response) {
	response = createStatusResponse3();
	if (request == response) {
//		m_log(logging::INFO) << "handleGetStatus3 will long poll, json=" << raw_request.body << std::endl;
		LongPollClient lpc;
		lpc.original_who        = who;
		lpc.original_request    = raw_request;
		lpc.original_jsonrpc_id = raw_js_request.getId();
...
bool WalletNode::handle_get_addresses3(http::Client *, http::RequestData &&,
                                       json_rpc::Request &&,
                                       api::walletd::GetAddresses::Request &&,
                                       api::walletd::GetAddresses::Response &response) {
...
bool WalletNode::handle_get_view_key3(http::Client *, http::RequestData &&,
                                      json_rpc::Request &&,
                                      api::walletd::GetViewKeyPair::Request &&,
                                      api::walletd::GetViewKeyPair::Response &response) {
...
bool WalletNode::handle_create_address_list3(http::Client *, http::RequestData &&,
                                             json_rpc::Request &&,
                                             api::walletd::CreateAddresses::Request &&request,
                                             api::walletd::CreateAddresses::Response &response) {
...
bool WalletNode::handle_get_balance3(http::Client *, http::RequestData &&,
                                     json_rpc::Request &&,
                                     api::walletd::GetBalance::Request &&request,
                                     api::walletd::GetBalance::Response &response) {
	if( request.height_or_depth < 0)
		request.height_or_depth = std::max(0, static_cast<api::HeightOrDepth>(m_wallet_state.get_tip_height()) + 1 + request.height_or_depth);
	// TODO - error if address does not exist
	response = m_wallet_state.get_balance(request.address, request.height_or_depth);
	return true;
}

bool WalletNode::handle_get_unspent3(http::Client *, http::RequestData &&,
                                     json_rpc::Request &&,
                                     api::walletd::GetUnspents::Request &&request,
                                     api::walletd::GetUnspents::Response &response) {
	if( request.height_or_depth < 0)
		request.height_or_depth = std::max(0, static_cast<api::HeightOrDepth>(m_wallet_state.get_tip_height()) + 1 + request.height_or_depth);
	// TODO - error if address does not exist
...
bool WalletNode::handle_get_transfers3(http::Client *, http::RequestData &&,
									   json_rpc::Request &&,
									   api::walletd::GetTransfers::Request &&request,
									   api::walletd::GetTransfers::Response &response) {
	response.next_to_height = request.to_height;
	response.next_from_height = request.from_height;
	response.blocks       = m_wallet_state.api_get_transfers(request.address, request.from_height, request.to_height, request.forward, request.desired_transactions_count);
...
bool WalletNode::handle_create_transaction3(http::Client *who, http::RequestData &&raw_request,
                                            json_rpc::Request &&raw_js_request,
                                            api::walletd::CreateTransaction::Request &&request,
                                            api::walletd::CreateTransaction::Response &response) {
	if( request.confirmed_height_or_depth < 0)
		request.confirmed_height_or_depth = std::max(0, static_cast<api::HeightOrDepth>(m_wallet_state.get_tip_height()) + 1 - request.confirmed_height_or_depth);
	if( request.fee_per_byte == 0 )
		request.fee_per_byte = m_last_node_status.recommended_fee_per_byte;
	if( request.fee_per_byte == 0 )
		throw json_rpc::Error(json_rpc::errInvalidParams, "'fee_per_byte' set to 0, and it is impossible to set it to 'status.recommended_fee_per_byte', because walletd never connected to bytecoind after it was restarted");
	AccountPublicAddress change_addr;
	AccountPublicAddress spend_addr;
	// We require change address, even if you are lucky and got zero change this time
	if( m_wallet_state.get_wallet().is_view_only())
		throw json_rpc::Error(json_rpc::errInvalidParams, "Unable to create transaction - view-only wallet contains no spend keys");
	...
	if (!request.spend_address.empty() && !m_wallet_state.get_currency().parse_account_address_string(request.spend_address, spend_addr))
		throw json_rpc::Error(json_rpc::errInvalidParams,
									"Failed to parse change address " + request.change_address);
	if( request.spend_address.empty() && !request.any_spend_address)
		throw json_rpc::Error(json_rpc::errInvalidParams, "Empty spend address requires setting 'any_spend_address':true for additional protection");
	if( !request.spend_address.empty() && request.any_spend_address)
		throw json_rpc::Error(json_rpc::errInvalidParams, "Non-empty spend address requires setting 'any_spend_address':false for additional protection");
	...
	UnspentSelector selector(m_wallet_state.get_currency(),
	                         m_wallet_state.api_get_unspent(
	                             request.spend_address, request.confirmed_height_or_depth));
	...
	for (auto &&tr : request.transaction.transfers) {
		if (tr.amount <= 0)  // Not an output
			throw json_rpc::Error(json_rpc::errInvalidParams, "Negative transfer amount " + std::to_string(tr.amount) + " for address " + tr.address);
	...
	for (auto aa : combined_outputs) {
		...
		decomposeAmount(aa.second, m_wallet_state.get_currency().default_dust_threshold, decomposed_amounts);
	...
	if (!selector.select_optimal_outputs(m_wallet_state.get_tip_height(), m_wallet_state.get_tip().timestamp,
	                                     ...
	                                     request.transaction.anonymity, sum_positive_transfers, total_outputs,
	                                     request.fee_per_byte, optimization, change)) {
        throw json_rpc::Error(json_rpc::errInvalidParams, "Not enough funds on selected addresses with desired confirmations");
	}
	...
	for (auto aa : combined_outputs) {
		std::vector<uint64_t> decomposed_amounts;
		decomposeAmount(aa.second, m_wallet_state.get_currency().default_dust_threshold,
		                decomposed_amounts);
	...
	if (m_inproc_node) {
		m_inproc_node->on_get_random_outputs3(nullptr, http::RequestData(raw_request), json_rpc::Request(),
		                                      std::move(ra_request), ra_response);
		std::unordered_map<PublicKey, WalletRecord> single_record = m_wallet_state.get_wallet().get_single_record(spend_addr);
		selector.add_mixed_inputs(m_wallet_state.get_wallet().get_view_secret_key(), request.any_spend_address ? m_wallet_state.get_wallet().get_records() : single_record, builder, request.transaction.anonymity,
		                          std::move(ra_response));
		Transaction tx           = builder.sign(m_wallet_state.get_wallet().get_tx_derivation_seed());
		response.binary_transaction  = seria::toBinary(tx);
		...
		if (request.save_history && !m_wallet_state.get_wallet().save_history(transaction_hash, history)) {
			m_log(logging::ERROR) << "Saving transaction history failed, proof of sending will be unavailable for tx="
	...
	http::RequestData new_request =
	    json_rpc::createRequest(api::bytecoind::url(), api::bytecoind::GetRandomOutputs::method(), ra_request);
	add_waiting_command(
	    who, std::move(raw_request), raw_js_request.getId(), std::move(new_request),
	    [=](WaitingClient &&wc, const http::ResponseData &random_response) mutable {
			try {
			    ...
			    api::bytecoind::GetRandomOutputs::Response ra_response;
			    json_resp.getResult(ra_response);
				std::unordered_map<PublicKey, WalletRecord> single_record = m_wallet_state.get_wallet().get_single_record(spend_addr);
			    selector.add_mixed_inputs(m_wallet_state.get_wallet().get_view_secret_key(), request.any_spend_address ? m_wallet_state.get_wallet().get_records() : single_record, builder, request.transaction.anonymity,
			                              std::move(ra_response));
			    tx              = builder.sign(m_wallet_state.get_wallet().get_tx_derivation_seed());
				last_response.binary_transaction = seria::toBinary(tx);
			    tx_hash         = get_transaction_hash(tx);
				if (request.save_history && !m_wallet_state.get_wallet().save_history(tx_hash, history)) {
					m_log(logging::ERROR)
							<< "Saving transaction history failed, proof of sending will not be available for tx="
...
//				m_log(logging::INFO) << "Replying with error"  << std::endl;
//			    last_response.transaction_hash = tx_hash;
				http::ResponseData last_http_response =
						json_rpc::createResponse(wc.original_request, last_response, wc.original_jsonrpc_id);
				wc.original_who->write(std::move(last_http_response));
			} catch (const std::exception & ex) {
				json_rpc::Response last_json_resp;
				last_json_resp.setId(wc.original_jsonrpc_id);
				last_json_resp.setError(json_rpc::Error(json_rpc::errInvalidParams, ex.what()));
...
bool WalletNode::handle_create_send_proof3(http::Client *, http::RequestData &&, json_rpc::Request &&,
							   api::walletd::CreateSendProof::Request && request,
	...
	for(auto && address : addresses){
		...
		if( m_wallet_state.api_create_proof(sp) )
			response.send_proofs.push_back(seria::toJsonValue(sp).to_string());
	}
	return true;
}
...
bool WalletNode::handle_send_transaction3(http::Client *who, http::RequestData &&raw_request,
	...
	if (m_inproc_node) {
		m_inproc_node->handle_send_transaction3(nullptr, std::move(raw_request), std::move(raw_js_request),
		                                             std::move(request), response);
		return true;
	}
	http::RequestData new_request;
	new_request.setBody(std::move(raw_request.body));  // We save on copying body here
	new_request.r.set_firstline("POST", api::bytecoind::url(), 1, 1);
	transient_transactions_counter += 1;
	add_waiting_command(who, std::move(raw_request), raw_js_request.getId(), std::move(new_request),
...
bool WalletNode::handle_get_transaction3(http::Client *, http::RequestData &&, json_rpc::Request &&,
								   api::walletd::GetTransaction::Request && req,
								   api::walletd::GetTransaction::Response & res){
	Transaction tx;
...
void WalletNode::send_next_waiting_command() {
	...
	m_command_request =
	    std::make_unique<http::Request>(m_commands_agent, std::move(m_waiting_command_requests.front().request), fun, e_fun);
}

void WalletNode::add_waiting_command(
    http::Client *who, http::RequestData &&original_request, const json_rpc::OptionalJsonValue &original_rpc_id,
    http::RequestData &&request, std::function<void(WalletNode::WaitingClient &&wc, http::ResponseData &&resp)> fun) {
	WaitingClient wc2;
...
void WalletNode::advance_long_poll() {
	...
	for (auto lit = m_long_poll_http_clients.begin(); lit != m_long_poll_http_clients.end();)
		if (lit->original_get_status != resp) {
			last_json_resp.setId(lit->original_jsonrpc_id);
			http::ResponseData last_http_response;
			last_http_response.r.headers.push_back({"Content-Type", "application/json; charset=utf-8"});
			last_http_response.r.status             = 200;
			...
			last_http_response.setBody(last_json_resp.getBody());
//			m_log(logging::INFO) << "advance_long_poll will reply to long poll json=" << last_http_response.body << std::endl;
