...
bool Node::process_json_rpc_request(http::Client *who, http::RequestData &&request, http::ResponseData &response) {
	...
	try {
		...
		auto it = m_jsonrpc_handlers.find(json_req.getMethod());
		if (it == m_jsonrpc_handlers.end()) {
			m_log(logging::INFO) << "jsonrpc request method not found - " << json_req.getMethod() << std::endl;
			throw json_rpc::Error(json_rpc::errMethodNotFound);
		}
//		m_log(logging::INFO) << "jsonrpc request method=" << json_req.getMethod() << std::endl;
...
bool Node::on_getblocktemplate(http::Client *who, http::RequestData &&raw_request,
                               json_rpc::Request &&raw_js_request, api::bytecoind::GetBlockTemplate::Request &&req,
	...
	m_log(logging::INFO) << "Node received getblocktemplate REQ transaction_pool_version=" << req.transaction_pool_version
	          << " top_block_hash=" << common::pod_to_hex(req.top_block_hash) << std::endl;
	m_log(logging::INFO) << "Node received getblocktemplate CUR transaction_pool_version=" << m_block_chain.get_tx_pool_version()
	          << " top_block_hash=" << common::pod_to_hex(m_block_chain.get_tip_bid()) << std::endl;
	if (sta.top_block_hash == m_block_chain.get_tip_bid() && sta.transaction_pool_version == m_block_chain.get_tx_pool_version()) {
//		m_log(logging::INFO) << "on_getblocktemplate will long poll, json=" << raw_request.body << std::endl;
		LongPollClient lpc;
...
void Node::getblocktemplate(const api::bytecoind::GetBlockTemplate::Request &req,
	...
	BinaryArray block_blob = seria::toBinary(block_template);
//	BinaryArray block_blob = toBinaryArray(block_template);
	PublicKey tx_pub_key   = getTransactionPublicKeyFromExtra(block_template.base_transaction.extra);
	...
	if (0 < req.reserve_size) {
		res.reserved_offset = static_cast<uint32_t>(slow_memmem(block_blob.data(), block_blob.size(), &tx_pub_key, sizeof(tx_pub_key)));
...
bool Node::on_get_currency_id(http::Client *, http::RequestData &&,
                              json_rpc::Request &&, api::bytecoind::GetCurrencyId::Request && /*req*/,
							  api::bytecoind::GetCurrencyId::Response &res) {
...
//		throw json_rpc::Error{CORE_RPC_ERROR_CODE_WRONG_BLOCKBLOB, "Wrong block blob 2"};
//	}
	seria::fromBinary(block_template, blockblob);
	RawBlock raw_block;
	...
	BinaryArray raw_msg           = LevinProtocol::sendMessage(NOTIFY_NEW_BLOCK::ID, LevinProtocol::encode(msg), false);
