		...
		if (it == m_jsonrpc_handlers.end()) {
			...
			if (WalletNode::m_jsonrpc_handlers.count(json_req.get_method()) != 0)
				throw json_rpc::Error(json_rpc::METHOD_NOT_FOUND,
				    "Method not found " + json_req.get_method() + " (attempt to call walletd method on bytecoind)");
...
void Node::getblocktemplate(const api::bytecoind::GetBlockTemplate::Request &req,
	...
	try {
		m_block_chain.create_mining_block_template(acc, blob_reserve, &block_template, &res.difficulty, &res.height);
	...
	if (req.reserve_size > 0) {
		const unsigned char *found =
		    common::slow_memmem(block_blob.data(), block_blob.size(), tx_pub_key.data, sizeof(tx_pub_key));
		if (!found) {
		...
		res.reserved_offset = static_cast<uint32_t>(found - block_blob.data());
		res.reserved_offset +=
		    sizeof(tx_pub_key) + 2;  // tag for TX_EXTRA_NONCE(1 byte), counter in TX_EXTRA_NONCE(1 byte)
		if (res.reserved_offset + req.reserve_size > block_blob.size()) {
			m_log(logging::ERROR) << "Failed to calculate offset for reserved bytes";
			throw json_rpc::Error{json_rpc::INTERNAL_ERROR, "Internal error: failed to create block template"};
		}
		for (size_t i = 0; i != req.reserve_size; ++i)
			invariant(block_blob.at(res.reserved_offset + i) == 00, "");
	}
