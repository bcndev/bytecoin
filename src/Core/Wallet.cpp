...
//	uint8_t version; // We moved version out of this struct, because with it other fields become unaligned
...
static void decrypt_key_pair(const EncryptedWalletRecord &r, PublicKey &pk, SecretKey &sk, Timestamp &ct,
                             const WalletKey &key) {
...
void Wallet::load_container_storage() {
	...
	uint64_t item_capacity = 0;
	uint64_t item_count    = 0;
	file->read(&version, 1);
	file->read(reinterpret_cast<char *>(&prefix), sizeof(prefix));
	file->read(reinterpret_cast<char *>(&item_capacity), sizeof(item_capacity));
	...
	decrypt_key_pair(prefix.encrypted_view_keys, m_view_public_key, m_view_secret_key, m_creation_timestamp, m_wallet_key);
	...
	item_count = std::min(item_count, item_capacity); // Protection against write shredding
	...
	for (size_t i = 0; i != item_count; ++i) {
		...
		decrypt_key_pair(all_encrypted[i], wallet_record.spend_public_key, wallet_record.spend_secret_key, wallet_record.creation_timestamp, m_wallet_key);
		...
		if( i < CHECK_KEYS_COUNT || i >= item_count - CHECK_KEYS_COUNT) { // check only first and last spend keys
			if (wallet_record.spend_secret_key != SecretKey{}) {
				if( !keys_match(wallet_record.spend_secret_key, wallet_record.spend_public_key) )
					throw Exception(api::WALLET_FILE_DECRYPT_ERROR, "Restored spend public key doesn't correspond to secret key");
			} else {
...
Wallet::Wallet(const std::string & path, const std::string & password, bool create):m_path(path), m_password(password) {
	...
	if( create ){
		...
		if( file.get() ) // opened ok
			throw Exception(api::WALLET_FILE_EXISTS, "Will not overwrite existing wallet - delete it first or specify another file " + path);
	...
	if (!is_view_only()) {
		...
		common::append(seed_data, std::begin(first_record.spend_secret_key.data), std::end(first_record.spend_secret_key.data));
...
void Wallet::save(const std::string & export_path, bool view_only){
	...
	ContainerStoragePrefix prefix{};
	encrypt_key_pair(prefix.encrypted_view_keys, m_view_public_key, m_view_secret_key, m_creation_timestamp,
					 m_wallet_key);
	...
	encrypt_key_pair(record, first_record.spend_public_key, view_only ? SecretKey{} : first_record.spend_secret_key, first_record.creation_timestamp, m_wallet_key);
	f.write(&record, sizeof(record));
	for (auto &&r : m_wallet_records) {
		if(r.second.spend_public_key == first_record.spend_public_key)
			continue;
		encrypt_key_pair(record, r.second.spend_public_key, view_only ? SecretKey{} : r.second.spend_secret_key, r.second.creation_timestamp, m_wallet_key);
...
void Wallet::export_view_only(const std::string & export_path){
	...
	if( export_file.get() ) // opened ok
		throw Exception(api::WALLET_FILE_EXISTS, "Will not overwrite existing wallet - delete it first or specify another file " + export_path);
...
std::pair<WalletRecord, bool> Wallet::generate_new_address(const SecretKey &sk, Timestamp ct) {
	...
	if (sk == SecretKey{})
		...
		if (!secret_key_to_public_key(sk, record.spend_public_key))
			throw Exception(101, "Generated keypair is invalid - sk=" + common::pod_to_hex(sk));
	}
...
std::vector<WalletRecord> Wallet::generate_new_addresses(const std::vector<SecretKey> & sks, Timestamp ct){
	...
	size_t append_pos = 1 + sizeof(ContainerStoragePrefix) + sizeof(uint64_t) * 2 + sizeof(EncryptedWalletRecord) * m_wallet_records.size();
	...
	for(auto && sk : sks){
		...
		EncryptedWalletRecord record;
		encrypt_key_pair(record, new_rit.first.spend_public_key, new_rit.first.spend_secret_key, new_rit.first.creation_timestamp, m_wallet_key);
...
std::unordered_map<PublicKey, WalletRecord> Wallet::get_single_record(const AccountPublicAddress & addr)const{
	...
	if( rit != m_wallet_records.end() && rit->second.spend_public_key == addr.spend_public_key && m_view_public_key == addr.view_public_key)
		result.insert(*rit);
	return result;
}
...
Wallet::History Wallet::load_history(const Hash &bid)const {
	...
	crypto::chacha8(hist.data() + sizeof(crypto::chacha8_iv), hist.size() - sizeof(crypto::chacha8_iv), m_history_key, *iv, dec.data());
