...
uint32_t BlockChainState::DeltaState::push_amount_output(Amount amount, UnlockMoment unlock_time, Height block_height, Timestamp block_unlock_timestamp, const PublicKey &pk) {
...
bool BlockChainState::DeltaState::read_amount_output(Amount amount, uint32_t global_index, UnlockMoment &unlock_time,
                                                     PublicKey &pk) const {
	uint32_t pg = m_parent_state->next_global_index_for_amount(amount);
	if (global_index < pg)
...
bool BlockChainState::check_standalone_consensus(const PreparedBlock & pb, api::BlockHeader &info,
                                                 const api::BlockHeader &prev_info) const {
	auto err = get_standalone_consensus_error(pb, info, prev_info);
	return err.empty();
}

std::string BlockChainState::get_standalone_consensus_error(const PreparedBlock & pb,
                                                            api::BlockHeader &info,
                                                            const api::BlockHeader &prev_info) const {
	...
	if( get_tip_height() != prev_info.height ) // Optimization for most common case
		calculate_consensus_values(get_tip_height() - prev_info.height, info.size_median, info.timestamp_median, info.timestamp_unlock);
	...
	for (auto &&raw_tx : pb.raw_block.transactions) {
		if (raw_tx.size() > m_currency.max_transaction_allowed_size(info.effective_size_median)) {
			//            log(Logging::INFO) << "Raw transaction size " << binary_transaction.size() << " is too big.";
	...
	if (!m_currency.get_block_reward(block.header.major_version, info.effective_size_median, 0, already_generated_coins,
		...
		//        log(Logging::WARNING) << "Block " << cachedBlock.getBlockHash() << " has too big cumulative size";
		return "CUMULATIVE_BLOCK_SIZE_TOO_BIG";
	}

	if (miner_reward != info.reward) {
		//        log(Logging::WARNING) << "Block reward mismatch for block " << cachedBlock.getBlockHash()
	...
	if (m_currency.is_in_checkpoint_zone(info.height)) {
		...
		Hash long_hash = pb.long_block_hash != Hash{} ? pb.long_block_hash : get_block_long_hash(block.header, m_hash_crypto_context);
...
void BlockChainState::calculate_consensus_values(Height height_delta, uint32_t & next_median_size, Timestamp & next_median_timestamp, Timestamp & next_unlock_timestamp)const{
	...
	if (window.second - window.first >= m_currency.timestamp_check_window) {
		std::vector<Timestamp> timestamps;
		...
		// unlike medianValue, here we select lesser of 2 middle values for even-sized array, so
...
bool BlockChainState::create_mining_block_template(BlockTemplate &b, const AccountPublicAddress &adr,
                                                   const BinaryArray &extra_nonce, Difficulty &difficulty,
                                                   Height &height) const {
	...
	if (b.major_version == 1) {
		...
		if (!appendMergeMiningTagToExtra(b.parent_block.base_transaction.extra, mm_tag)) {
			m_log(logging::ERROR) << logging::BrightRed
	...
	// technically we should give unlock timestamp of next block, but more conservative also works
	...
	for (; !pool_hashes.empty(); pool_hashes.pop_back()) {
		...
		size_t tx_size = seria::binarySize(tit->second);  // TODO - remember size in tx index
		if (txs_size + tx_size > block_size_limit)
			continue;
		Amount single_fee = 0;
		bool fatal        = false;
		BlockGlobalIndices global_indices;
		std::string result = redo_transaction_get_error(false, tit->second, &memory_state, global_indices,
		                                                true, single_fee, fatal);  // nextMedianTimestamp
		if (!result.empty()) {
	...
	size_t cumulative_size   = txs_size + seria::binarySize(b.base_transaction);
	...
	for (size_t try_count = 0; try_count < TRIES_COUNT; ++try_count) {
		...
		size_t coinbase_blob_size = seria::binarySize(b.base_transaction);
		...
		if (coinbase_blob_size < cumulative_size - txs_size) {
			...
			if (cumulative_size != txs_size + seria::binarySize(b.base_transaction)) {
				if (cumulative_size + 1 != txs_size + seria::binarySize(b.base_transaction)) {
					m_log(logging::ERROR) << logging::BrightRed
					    << "unexpected case: cumulative_size=" << cumulative_size
					    << " + 1 is not equal txs_cumulative_size=" << txs_size
					    << " + get_object_blobsize(b.baseTransaction)=" << seria::binarySize(b.base_transaction);
					return false;
				}

				b.base_transaction.extra.resize(b.base_transaction.extra.size() - 1);
				if (cumulative_size != txs_size + seria::binarySize(b.base_transaction)) {
					// ooh, not lucky, -1 makes varint-counter size smaller, in that case we continue to grow with
					// cumulative_size
					m_log(logging::TRACE) << logging::BrightRed
					    << "Miner tx creation have no luck with delta_extra size = " << delta << " and " << delta - 1;
					cumulative_size += delta - 1;
					continue;
				}

				m_log(logging::DEBUGGING) << logging::BrightGreen
				    << "Setting extra for block: " << b.base_transaction.extra.size() << ", try_count=" << try_count;
			}
		}
		if (cumulative_size != txs_size + seria::binarySize(b.base_transaction)) {
			m_log(logging::ERROR) << logging::BrightRed
...
BroadcastAction BlockChainState::add_mined_block(const BinaryArray &raw_block_template, RawBlock &raw_block,
                                                 api::BlockHeader &info) {
	BlockTemplate block_template;
	seria::fromBinary(block_template, raw_block_template);
	...
	for (const auto &tx_hash : block_template.transaction_hashes) {
		...
		raw_block.transactions.emplace_back(seria::toBinary(*tx));
	}
	PreparedBlock pb(std::move(raw_block), nullptr);
...
BroadcastAction BlockChainState::add_transaction(const Hash & tid, const Transaction & tx, Height unlock_height, Timestamp unlock_timestamp, size_t max_pool_complexity, bool check_sigs){
	const size_t my_size = seria::binarySize(tx);
	...
	// Only good transactions are recorded in tx_first_seen, because they require space there
	...
	for (auto &&ki : memory_state.get_keyimages()) {
		...
		const size_t other_size       = seria::binarySize(other_tx);
		const Amount other_fee_per_byte = other_fee / other_size;
		if (my_fee_per_byte < other_fee_per_byte)
			return BroadcastAction::NOTHING;
		if (my_fee_per_byte == other_fee_per_byte &&
			tid < tit->second)  // Deterministic behaviour so tx pools have tendency to stay the same
			return BroadcastAction::NOTHING;
...
void BlockChainState::remove_from_pool(Hash tid) {
	...
	// We do not increment m_tx_pool_version, because removing tx from pool is always followed by reset or increment
}
...
void BlockChainState::update_first_seen_timestamp(const Hash & tid, Timestamp now){
	auto key = FIRST_SEEN_PREFIX + DB::to_binary_key(tid.data, sizeof(tid.data));
	if( now != 0)
		m_db.put(key, seria::toBinary(now), false);
	else
		m_db.del(key, false);
}
...
RingCheckerMulticore::RingCheckerMulticore(){
	...
	std::cout << "Starting multicore ring checker using " << th_count << "/" << std::thread::hardware_concurrency() << " cpus" << std::endl;
...
void RingCheckerMulticore::thread_run(){
	while(true){
		...
		bool result = check_ring_signature(arg.tx_prefix_hash, arg.key_image, output_key_pointers.data(),
								  output_key_pointers.size(), arg.signatures.data(),
								  true, &key_corrupted);
		{
			std::unique_lock<std::mutex> lock(mu);
...
std::string RingCheckerMulticore::start_work_get_error(IBlockChainState * state, const Currency & currency, const Block & block, Height unlock_height, Timestamp unlock_timestamp){
	{
		std::unique_lock<std::mutex> lock(mu);
		args.clear();
		errors.clear();
	...
	for(auto && transaction : block.transactions){
		...
		for (const auto &input : transaction.inputs) {
			if (input.type() == typeid(CoinbaseInput)) {
				...
				// As soon as first arg is ready, other thread can start work while we continue reading from slow DB
				total_counter += 1;
				std::unique_lock<std::mutex> lock(mu);
...
std::string BlockChainState::redo_transaction_get_error(bool generating, const Transaction &transaction,
                                                        DeltaState *delta_state, BlockGlobalIndices &global_indices,
	...
	fatal = false; // for now we do not distinguish between fatal and non-fatal errors
	if (!error.empty())
		return error;
	Hash tx_prefix_hash;
	if (check_sigs)
			...
			if (check_sigs || check_outputs) {
				...
				for (size_t i = 0; i != global_indexes.size(); ++i) {
					...
					if (!tx_delta.read_amount_output(in.amount, global_indexes[i], unlock_time, output_keys[i]))
						return "INPUT_INVALID_GLOBAL_INDEX";
					if (!m_currency.is_transaction_spend_time_unlocked(unlock_time, delta_state->get_block_height(),
	...
	for (const auto &output : transaction.outputs) {
		if (output.target.type() == typeid(KeyOutput)) {
			const KeyOutput &key_output = boost::get<KeyOutput>(output.target);
			auto global_index = tx_delta.push_amount_output(output.amount, transaction.unlock_time, 0, 0, key_output.key); // DeltaState ignores unlock point
			my_indices.push_back(global_index);
		} else
			return "OUTPUT_UNKNOWN_TYPE";
	}
	tx_delta.apply(delta_state); // delta_state might be memory pool, we protect it from half-modification
	return std::string();
}
...
bool BlockChainState::redo_block(const Block &block, const api::BlockHeader &info, DeltaState *delta_state,
                                 BlockGlobalIndices &global_indices) const {
    // We will need info.timestamp_unlock after first hard fork
	Amount fee         = 0;
	bool fatal         = false;
	std::string result = redo_transaction_get_error(true, block.header.base_transaction, delta_state, global_indices,
	                                                false, fee, fatal);
	if (!result.empty())
		return false;
	SignedAmount sum_fee = 0;
	for (auto tit = block.transactions.begin(); tit != block.transactions.end(); ++tit) {
		std::string result = redo_transaction_get_error(false, *tit, delta_state, global_indices,
		                                                false, fee, fatal);
		if (!result.empty())
			return false;
		sum_fee += fee;
...
bool BlockChainState::redo_block(const Hash &bhash, const Block &block, const api::BlockHeader &info) {
	...
	BinaryArray ba = seria::toBinary(global_indices);
	...
	if (info.height % 100 == 0)
		std::cout << "redo_block {" << block.transactions.size() << "} height=" << info.height
					<< " bid=" << common::pod_to_hex(bhash)
		          << " old tip_bid=" << common::pod_to_hex(get_tip_bid()) << std::endl;
	return true;
}
...
void BlockChainState::undo_block(const Hash &bhash, const Block &block, Height height) {
	...
	for (auto tit = block.transactions.begin(); tit != block.transactions.end(); ++tit) {
		if(m_memory_state_total_complexity < MAX_POOL_COMPLEXITY * 2) {
			...
			// undo is rare, otherwise we'd optimize to use block.header.transactionHashes
			add_transaction(thash, *tit, height, get_tip().timestamp + m_currency.block_future_time_limit * 2, std::numeric_limits<size_t>::max(), false);
			// we use increased timestamp so that just unlocked transactions will not be thrown out of the pool
			// we set no limit on complexity to overshoot MAX_POOL_COMPLEXITY * 2 and stop doing work in case of large undo
		}
	}
}
...
bool BlockChainState::read_block_output_global_indices(const Hash &bid, BlockGlobalIndices &indices) const {
	...
	if (!m_db.get(key, rb))
		return false;
	seria::fromBinary(indices, rb);
	return true;
}

std::vector<api::Output> BlockChainState::get_outputs_by_amount(Amount amount, size_t anonymity, Height height,
...
uint32_t BlockChainState::push_amount_output(Amount amount, UnlockMoment unlock_time, Height block_height, Timestamp block_unlock_timestamp, const PublicKey &pk) {
	...
	BinaryArray ba   = seria::toBinary(std::make_pair(unlock_time, pk));
	m_db.put(key, ba, true);

	if (create_unlock_index && !m_currency.is_transaction_spend_time_unlocked(unlock_time, block_height, block_unlock_timestamp)) {
		std::string unkey;
		...
		unkey += common::write_varint_sqlite4(amount) + common::write_varint_sqlite4(clamped_unlock_time) + common::write_varint_sqlite4(my_gi);
...
void BlockChainState::pop_amount_output(Amount amount, UnlockMoment unlock_time, const PublicKey &pk) {
	...
	if (create_unlock_index && !m_currency.is_transaction_spend_time_unlocked(unlock_time, get_tip_height(), get_tip().timestamp_unlock)) {
		std::string unkey;
		uint32_t clamped_unlock_time = static_cast<uint32_t>(std::min<UnlockMoment>(unlock_time, 0xFFFFFFFF));
		if (m_currency.is_transaction_spend_time_block(unlock_time))
			unkey = UNLOCK_BLOCK_PREFIX;
			...
			unkey = UNLOCK_TIME_PREFIX;
		unkey += common::write_varint_sqlite4(amount) + common::write_varint_sqlite4(clamped_unlock_time) + common::write_varint_sqlite4(next_gi);
		m_db.del(unkey, true);
	}
}
...
bool BlockChainState::read_amount_output(Amount amount, uint32_t global_index, UnlockMoment &unlock_time,
                                         PublicKey &pk) const {
