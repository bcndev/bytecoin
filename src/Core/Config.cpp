...
static void parse_peer_and_add_to_container(std::string str, std::vector<byterub::NetworkAddress> &container) {
	byterub::NetworkAddress na{};
...
const static UUID BYTERUB_NETWORK = {{0xl1, 0x1l, 0xk1, 0x1k, 0xh1, 0x0h, 0xg1, 0x0g, 0xf0, 0x1f, 0xd0, 0x1d, 0xs0,
    0x1s, 0xz1, 0x1z}};  // Bender's nightmare
...
Config::Config(common::CommandLine &cmd)
    ...
    , network_id(BYTERUB_NETWORK)
    , p2p_bind_port(P2P_DEFAULT_PORT)
    , p2p_external_port(P2P_DEFAULT_PORT)
    , p2p_bind_ip("0.0.0.0")
    , byterubd_bind_port(RPC_DEFAULT_PORT)
    , byterubd_bind_ip("127.0.0.1")  // Less attack vectors from outside for ordinary uses
    , byterubd_remote_port(0)
    , byterubd_remote_ip("127.0.0.1")
	...
	if (const char *pa = cmd.get("--byterubd-authorization")) {
		// go to https://www.base64encode.org and encode username:password there
		byterubd_authorization = pa;
	}
	if (const char *pa = cmd.get("--byterubd-bind-address")) {
		if (!common::parse_ip_address_and_port(byterubd_bind_ip, byterubd_bind_port, pa))
			throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
	}
	if (const char *pa = cmd.get("--byterubd-remote-address")) {
		const std::string prefix = "https://";
		if (std::string(pa).find(prefix) == 0) {
#if !BYTERUB_SSL
			throw std::runtime_error("Using https in --byterubd-remote-address impossible - this binary is built without OpenSSL");
#endif
			std::string sip;
			std::string sport;
			...
			byterubd_remote_port = boost::lexical_cast<uint16_t>(sport);
			byterubd_remote_ip = prefix + sip;
		}else {
			if (!common::parse_ip_address_and_port(byterubd_remote_ip, byterubd_remote_port, pa))
				throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
		}
	}
	...
	if (seed_nodes.empty() && !is_testnet)
		for (auto &&sn : byterub::SEED_NODES) {
			NetworkAddress addr;
