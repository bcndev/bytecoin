...
WalletSync::WalletSync(logging::ILogger &log, const Config &config, WalletState &wallet_state, std::function<void()> state_changed_handler)
    : m_state_changed_handler(state_changed_handler)
    , m_log(log, "WalletSync")
    , m_status_timer(std::bind(&WalletSync::send_get_status, this))
    , m_sync_agent(config.bytecoind_remote_ip, config.bytecoind_remote_port ? config.bytecoind_remote_port : config.bytecoind_bind_port)
    , m_commands_agent(config.bytecoind_remote_ip, config.bytecoind_remote_port ? config.bytecoind_remote_port : config.bytecoind_bind_port)
...
void WalletSync::send_get_status() {
	...
	json_send_raw_req.setMethod(api::bytecoind::GetStatus::method());
	json_send_raw_req.setParams(req);
	...
	m_sync_request.reset( new http::Request(m_sync_agent, std::move(req_header),
		[&](http::ResponseData &&response) {
			...
			json_rpc::parseResponse(response.body, resp);
...
void WalletSync::advance_sync() {
	...
	if( prevent_sleep && m_wallet_state.get_tip().timestamp > now - m_wallet_state.get_currency().block_future_time_limit * 2)
...
void WalletSync::send_sync_pool() {
	...
	req_header.setBody(seria::toBinaryStr(msg));
	m_sync_request =
	    std::make_unique<http::Request>(m_sync_agent, std::move(req_header),
			[&](http::ResponseData &&response) {
				m_sync_request.reset();
				api::bytecoind::SyncMemPool::Response resp;
				seria::fromBinary(resp, response.body);
...
void WalletSync::send_get_blocks() {
	...
	m_sync_request =
	    std::make_unique<http::Request>(m_sync_agent, std::move(req_header),
	                                    ...
	                                    [&](std::string err) { m_status_timer.once(STATUS_ERROR_PERIOD); });
