...
public:
	...
	bool internal_import(Height count); // import some existing blocks from inside DB
...
protected:
	...
	virtual bool check_standalone_consensus(const PreparedBlock & pb, api::BlockHeader &info,
	                                        const api::BlockHeader &prev_info) const = 0;
	...
	bool redo_block(const Hash &bhash, const RawBlock &raw_block, const Block &block, const api::BlockHeader &info, const Hash & base_transaction_hash);
...
private:
	...
	bool reorganize_blocks(const Hash &switch_to_chain, const PreparedBlock & recent_pb, const api::BlockHeader &recent_info);
