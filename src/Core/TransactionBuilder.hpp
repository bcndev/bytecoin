...
public:
	...
	size_t add_input(const AccountKeys &sender_keys, api::Output real_output,
	...
	static crypto::KeyPair deterministic_keys_from_seed(const Transaction & tx, const Hash & tx_derivation_seed);
	...
	static bool derive_public_key(const AccountPublicAddress &to, const SecretKey &tx_key, size_t output_index,
...
class UnspentSelector {
	...
	typedef std::map<size_t, std::vector<api::Output>> DustCoins;
	Unspents m_unspents;
...
public:
	...
	void add_mixed_inputs(const SecretKey & view_secret_key, const std::unordered_map<PublicKey, WalletRecord> & wallet_records, TransactionBuilder &builder, uint32_t anonymity,
