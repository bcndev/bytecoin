...
bool Block::to_raw_block(RawBlock &raw_block) const {
	try {
		...
		raw_block.block              = seria::toBinary(bheader);
		raw_block.transactions.resize(0);
		raw_block.transactions.reserve(transactions.size());
		for (auto &&transaction : transactions) {
			BinaryArray rawTransaction = seria::toBinary(transaction);
...
PreparedBlock::PreparedBlock(BinaryArray && ba, crypto::CryptoNightContext * context):block_data(std::move(ba)){
	seria::fromBinary(raw_block, block_data);
	if (block.from_raw_block(raw_block))
		bid = bytecoin::get_block_hash(block.header);
	if (block.header.major_version >= 2)
		parent_block_size = seria::binarySize(block.header.parent_block);
	coinbase_tx_size = seria::binarySize(block.header.base_transaction);
...
PreparedBlock::PreparedBlock(RawBlock && rba, crypto::CryptoNightContext * context):raw_block(rba){
	block_data = seria::toBinary(raw_block);
	if (block.from_raw_block(raw_block))
		bid = bytecoin::get_block_hash(block.header);
	if (block.header.major_version >= 2)
		parent_block_size = seria::binarySize(block.header.parent_block);
...
BlockChain::BlockChain(const Hash &genesis_bid, const std::string &coin_folder)
    ...
    , m_coin_folder(coin_folder)
    , m_db(coin_folder + "/blockchain") {
	std::string version;
	m_db.get("$version", version);
	if (version != version_current){
        std::cout << "Data format changed, old version=" << version << " current version=" << version_current << ", deleting bytecoind cache..." << std::endl;
        ...
        for( DB::Cursor cur = m_db.rbegin(std::string()); !cur.end(); ){
			if( (erased + skipped) % 1000000 == 0)
        		std::cout << "Processed " << (erased + skipped)/1000000 << "/" << (total_items + 999999)/1000000 << " million DB records" << std::endl;
        	if( cur.get_suffix().find(TIP_CHAIN_PREFIX + previous_versions[0] + "/") == 0 ){
        		cur.next();
				skipped += 1;
        		continue; // chain
			}
        	if( cur.get_suffix().find(BLOCK_PREFIX) == 0 && cur.get_suffix().substr(cur.get_suffix().size() - BLOCK_SUFFIX.size()) == BLOCK_SUFFIX ){
        		Hash bid;
	...
	m_internal_import_known_height = cur2.end() ? 0 : boost::lexical_cast<Height>(common::read_varint_sqlite4(cur2.get_suffix()));
...
BroadcastAction BlockChain::add_block(const PreparedBlock & pb, api::BlockHeader &info) {
	...
	try {
		if( !have_block )
			store_block(pb.bid, pb.block_data);  // Do not commit between here and reorganize_blocks or invariant might be dead
		...
		std::cout << "Exception while reorganizing blockchain, probably out of disk space ex.what=" << ex.what() << std::endl;
		std::exit(api::BYTECOIND_DATABASE_ERROR); // TODO - rollback last transaction, reset state to last transaction state
	}
...
bool BlockChain::reorganize_blocks(const Hash &switch_to_chain, const PreparedBlock & recent_pb,
	...
	while (m_tip_bid != common) {
		...
		if (read_chain(m_tip_height) != m_tip_bid)
			throw std::logic_error("Invariant dead - after undo tip does not match read_chain" +
                                             common::pod_to_hex(m_tip_bid));
	}
	...
	while (!chain2.empty()) {
		...
		if (chha == recent_pb.bid) {
			...
			if (!redo_block(recent_pb.bid, recent_pb.raw_block, recent_pb.block, recent_info, recent_pb.base_transaction_hash))
                // invalid block on longest subchain, make no attempt to download the rest
			...
			if (!read_block(chha, raw_block) || !block.from_raw_block(raw_block))
				return false;  // Strange, we checked has_block, somehow "bad block" got into DB. TODO - throw?
			if (block.header.previous_block_hash != m_tip_bid)
				throw std::logic_error("Unexpected block prev, invariant dead");
			api::BlockHeader info = read_header(chha);
			// if redo fails, we will forever stuck on this block until longer chain appears, that does not include it
...
Hash BlockChain::get_common_block(const Hash &bid1, const Hash &bid2, std::vector<Hash> *chain1,
                                  std::vector<Hash> *chain2) const {
	Hash hid1            = bid1;
	Hash hid2            = bid2;
...
std::vector<Hash> BlockChain::get_sync_headers_chain(const std::vector<Hash> &locator, Height &start_height,
...
namespace seria {
void serMembers(APIRawBlockHeightDifficulty &v, ISeria &s) {
...
namespace seria {
void serMembers(APITransactionPos &v, ISeria &s) {
...
bool BlockChain::read_transaction(const Hash &tid, Transaction & tx, Height & height, size_t &index_in_block)const{
	...
	for(DB::Cursor cur = m_db.begin(txkey); !cur.end(); cur.next()) {
		...
		index_in_block = common::read_varint_sqlite4(be, en);
		Hash bid;
...
bool BlockChain::redo_block(const Hash &bhash, const RawBlock &raw_block, const Block &block,
                            const api::BlockHeader &info, const Hash & base_transaction_hash) {
	if (!redo_block(bhash, block, info))
		return false;
	auto tikey =
	    TIMESTAMP_BLOCK_PREFIX + common::write_varint_sqlite4(info.timestamp) + common::write_varint_sqlite4(info.height);
	m_db.put(tikey, std::string(), true);

	auto bkey = TRANSATION_PREFIX + DB::to_binary_key(base_transaction_hash.data, TRANSACTION_PREFIX_BYTES) + common::write_varint_sqlite4(info.height) + common::write_varint_sqlite4(0);
	...
	for (size_t tx_index = 0; tx_index != block.transactions.size(); ++tx_index) {
		...
		bkey = TRANSATION_PREFIX + DB::to_binary_key(tid.data, TRANSACTION_PREFIX_BYTES) + common::write_varint_sqlite4(info.height) + common::write_varint_sqlite4(tx_index + 1);
		m_db.put(bkey, std::string(), true);
	}
...
void BlockChain::undo_block(const Hash &bhash, const RawBlock &, const Block &block, Height height) {
	...
	auto tikey =
	    TIMESTAMP_BLOCK_PREFIX + common::write_varint_sqlite4(block.header.timestamp) + common::write_varint_sqlite4(height);
	m_db.del(tikey, true);

	Hash tid = get_transaction_hash(block.header.base_transaction);
	auto bkey = TRANSATION_PREFIX + DB::to_binary_key(tid.data, TRANSACTION_PREFIX_BYTES) + common::write_varint_sqlite4(height) + common::write_varint_sqlite4(0);
	m_db.del(bkey, true);
	for (size_t tx_index = 0; tx_index != block.transactions.size(); ++tx_index) {
		tid = block.header.transaction_hashes.at(tx_index);
		bkey = TRANSATION_PREFIX + DB::to_binary_key(tid.data, TRANSACTION_PREFIX_BYTES) + common::write_varint_sqlite4(height) + common::write_varint_sqlite4(tx_index + 1);
		m_db.del(bkey, true);
	}
}
...
void BlockChain::read_tip() {
	...
	seria::fromBinary(m_tip_bid, cur2.get_value_array());
...
void BlockChain::push_chain(Hash bid, Difficulty cumulative_difficulty) {
	m_tip_height += 1;
	BinaryArray ba = seria::toBinary(bid);
...
void BlockChain::modify_children_counter(Difficulty cd, const Hash & bid, int delta){
	...
	if( counter == 1 ){
		...
		BinaryArray ba = seria::toBinary(counter);
...
void BlockChain::test_print_structure()const{
	...
	for(DB::Cursor cur = m_db.begin(CHILDREN_PREFIX); !cur.end(); cur.next()) {
		...
		uint32_t counter = 1;
		seria::fromBinary(counter, cur.get_value_array());
...
bool BlockChain::read_next_internal_block(Hash & bid)const {
	BinaryArray ba;
	if (!m_db.get(TIP_CHAIN_PREFIX + previous_versions[0] + "/" + common::write_varint_sqlite4(get_tip_height() + 1), ba))
		return false;
	seria::fromBinary(bid, ba);
	return true;
}

bool BlockChain::internal_import(Height count){
	while(true){
		Hash bid;
